...
MainWindow::MainWindow(QWidget *parent)
    ...
    , unreadCount(0)
{
    qDebug() << "[App] MainWindow constructor called";
    ui->setupUi(this);

    // --- SERIAL PORT INITIALIZATION ---
    serialPort = new QSerialPort(this);
    serialPort->setPortName("COM5"); // Set your Arduino COM port here
    serialPort->setBaudRate(QSerialPort::Baud9600); // Set your Arduino baud rate here
    if (serialPort->open(QIODevice::ReadWrite)) {
        connect(serialPort, &QSerialPort::readyRead, this, &MainWindow::onSerialDataReceived);
        serialPortStatusMessage = "✅ Serial port opened for RFID";
        serialPortStatusIsOk = true;
        qDebug() << "[Serial] Port COM5 opened at 9600 baud.";
    } else {
        serialPortStatusMessage = "❌ Failed to open serial port for RFID";
        serialPortStatusIsOk = false;
        qDebug() << "[Serial] Failed to open port COM5: " << serialPort->errorString();
    }
    // --- END SERIAL PORT INITIALIZATION ---


    ui->setupUi(this);
    // Remplit la comboBox de planification prédictive avec les noms de projets
    remplirComboBoxSelectProject();
    // Connecte le signal de sélection du projet au slot de remplissage des champs
    connect(ui->comboBox_select_project, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &MainWindow::on_comboBox_select_project_currentIndexChanged);
...
void MainWindow::addNotification(int projectId, const QString& type, const QString& message, int relatedEntityId, const QString& relatedEntityType)
{
    ...
    query.bindValue(":projectId", projectId > 0 ? projectId : QVariant(0));
    query.bindValue(":type", type);
    query.bindValue(":message", message);
    query.bindValue(":relatedEntityId", relatedEntityId > 0 ? relatedEntityId : QVariant(0));
    query.bindValue(":relatedEntityType", relatedEntityType.isEmpty() ? QVariant(QString()) : relatedEntityType);
...
void MainWindow::remplirComboBoxSelectProject() {
    if (!ui->comboBox_select_project) return;
    ui->comboBox_select_project->clear();
    QSqlQuery query("SELECT NOM_PR FROM PROJET");
    while (query.next()) {
        QString nom = query.value(0).toString();
        ui->comboBox_select_project->addItem(nom);
    }
}

// Slot : Remplit les champs titre, complexité, statut selon le projet sélectionné
void MainWindow::on_comboBox_select_project_currentIndexChanged(int index) {
    if (index < 0) {
        ui->lineEdit_title->clear();
        ui->lineEdit_complexity->clear();
        ui->lineEdit_status->clear();
        return;
    }
    QString nom = ui->comboBox_select_project->itemText(index);
    QSqlQuery query;
    query.prepare("SELECT NOM_PR, COMPLEXITE, STATUT_PR FROM PROJET WHERE NOM_PR = :nom");
    query.bindValue(":nom", nom);
    if (query.exec() && query.next()) {
        ui->lineEdit_title->setText(query.value(0).toString());
        ui->lineEdit_complexity->setText(query.value(1).toString());
        ui->lineEdit_status->setText(query.value(2).toString());
    } else {
        ui->lineEdit_title->clear();
        ui->lineEdit_complexity->clear();
        ui->lineEdit_status->clear();
...
// Show status bar message about Arduino connection after window is displayed
#include <QMessageBox>
#include <QTimer>
#include <QSerialPort>
...
void MainWindow::showEvent(QShowEvent* event) {
    ...
    QTimer::singleShot(0, this, [this]() {
        if (isArduinoConnected()) {
            QMessageBox::information(this, "Arduino Connection", "✅ Arduino is connected");
        } else {
            QMessageBox::critical(this, "Arduino Connection", "❌ Arduino not found");
        }
        // Show serial port status popup after window is visible
        if (!serialPortStatusMessage.isEmpty()) {
            if (serialPortStatusIsOk) {
                QMessageBox::information(this, "Serial Port", serialPortStatusMessage);
            } else {
                QMessageBox::critical(this, "Serial Port", serialPortStatusMessage);
            }
        }
    });
}
...
// Onglet Planification Prédictive : bouton Prédire

// --- Show Consultant Info by UID ---
void MainWindow::showConsultantInfo(const QString& uid) {
    qDebug() << "[DEBUG] Entered showConsultantInfo for UID:" << uid;
    // Normalize UID: remove spaces and make uppercase
    QString normalizedUid = uid;
    normalizedUid.replace(" ", "");
    normalizedUid = normalizedUid.toUpper();
    qDebug() << "[RFID] Normalized scanned UID:" << normalizedUid << ", length:" << normalizedUid.length();

    // DEBUG: Print all columns for each row in CONSULTANT
    QSqlQuery allColQuery("SELECT * FROM CONSULTANT");
    QSqlRecord rec = allColQuery.record();
    while (allColQuery.next()) {
        QStringList rowData;
        for (int i = 0; i < rec.count(); ++i) {
            rowData << rec.fieldName(i) + ": " + allColQuery.value(i).toString();
        }
        qDebug() << "[RFID] DB ROW:" << rowData.join(" | ");
    }

    // DEBUG: Print all normalized RFID_CARDs from DB
    QSqlQuery allQuery("SELECT RFID_CARD FROM CONSULTANT");
    while (allQuery.next()) {
        QString dbUid = allQuery.value(0).toString();
        QString dbNormalized = dbUid;
        dbNormalized.replace(" ", "");
        dbNormalized = dbNormalized.toUpper();
        qDebug() << "[RFID] DB UID:" << dbUid << ", normalized:" << dbNormalized << ", length:" << dbNormalized.length();
    }

    QSqlQuery query;
    query.prepare("SELECT ID_CON, NOM_CON, PRENOM_CON FROM CONSULTANT WHERE REPLACE(UPPER(RFID_CARD), ' ', '') = :uid");
    query.bindValue(":uid", normalizedUid);
    if (query.exec() && query.next()) {
        int consultantId = query.value(0).toInt();
        QString nom = query.value(1).toString();
        QString prenom = query.value(2).toString();
        QString fullName = nom + " " + prenom;
        // Count number of projects linked to this consultant
        int projectCount = 0;
        QSqlQuery projectQuery;
        projectQuery.prepare("SELECT COUNT(*) FROM PROJET P JOIN CLIENT C ON P.ID_CLIENT = C.ID_CLI WHERE C.ID_CON = :cid");
        projectQuery.bindValue(":cid", consultantId);
        if (projectQuery.exec() && projectQuery.next()) {
            projectCount = projectQuery.value(0).toInt();
        }
        // Prepare consultant info
        QString info = QString("Consultant trouvé :\nNom complet : %1\nUID : %2\nNombre de projets liés : %3")
            .arg(fullName).arg(uid).arg(projectCount);
        // Show popup on PC
        QMessageBox::information(this, "Consultant Info", info);
        qDebug() << "[DEBUG] After popup, about to check serial port for sending.";
        qDebug() << "[DEBUG] Checking serialPort pointer:" << (serialPort != nullptr) << "isOpen:" << (serialPort && serialPort->isOpen());
        // Send to Arduino for OLED display (only once)
        if (serialPort && serialPort->isOpen()) {
            QString oledMsg = QString("Nom: %1\nUID: %2\nProjets: %3\nEND\n").arg(fullName).arg(uid).arg(projectCount);
            serialPort->write(oledMsg.toUtf8());
            qDebug() << "[SERIAL] Sent to Arduino:" << oledMsg;
        }
    } else {
        QMessageBox::warning(this, "Consultant Info", "Aucun consultant trouvé pour ce badge : " + uid);
    }
}

// --- RFID Serial Data Handler ---
void MainWindow::onSerialDataReceived() {
    if (!serialPort) return;
    serialBuffer.append(serialPort->readAll());
    while (serialBuffer.contains('\n')) {
        int newlineIdx = serialBuffer.indexOf('\n');
        QByteArray line = serialBuffer.left(newlineIdx).trimmed();
        serialBuffer.remove(0, newlineIdx + 1);
        QString strLine = QString::fromUtf8(line);
        qDebug() << "[Serial] Received line:" << strLine; // Log every line
        if (strLine.startsWith("Card UID:")) {
            QString uid = strLine.mid(QString("Card UID:").length()).trimmed();
            qDebug() << "[Serial] RFID UID received:" << uid;
            showConsultantInfo(uid);
        }
    }
}
...
//
// Arduino Serial Monitor Test Guidance:
// - Open Arduino IDE Serial Monitor or PuTTY.
// - Set to COM5, 9600 baud.
// - Scan a card: you should see the UID as a line ending with \n.
// - If you do not see anything, check Arduino wiring and code.


// Slot pour le bouton Prédire
void MainWindow::on_pushButton_predict_clicked() {
    QString titre = ui->lineEdit_title->text();
    QString complexite = ui->lineEdit_complexity->text();
    QString statut = ui->lineEdit_status->text();
    QString budget = "";
    QString start_date = "";
    QString end_date = "";

    // Appel au script Python pour obtenir la prédiction IA
    QProcess process;
    QString python = "python";
    QString script = "C:/Users/moham/OneDrive/Desktop/version_ia/predict_ai.py";
    qDebug() << "Chemin script Python:" << script;
    QStringList arguments;
    arguments << script << titre << complexite << statut << budget << start_date << end_date;

    qDebug() << "Commande Python:" << python << arguments;
    process.start(python, arguments);
    process.waitForFinished();

    QString output = QString::fromUtf8(process.readAllStandardOutput()).trimmed();
    QString errorOutput = QString::fromUtf8(process.readAllStandardError()).trimmed();
    qDebug() << "Raw AI output:" << output;
    qDebug() << "Erreur Python:" << errorOutput;
    QStringList results = output.split("|", Qt::SkipEmptyParts);
    for (int i = 0; i < results.size(); ++i) results[i] = results[i].trimmed();

    // Direct access to labels
    qDebug() << "label_predicted_duration:" << ui->label_predicted_duration;
    qDebug() << "label_risk:" << ui->label_risk;
    qDebug() << "label_advice:" << ui->label_advice;

    if (results.size() >= 3) {
        if (ui->label_predicted_duration) ui->label_predicted_duration->setText("Durée Prédite : " + results[0]);
        if (ui->label_risk) ui->label_risk->setText("Risque de Retard : " + results[1]);
        if (ui->label_advice) ui->label_advice->setText("Conseil : " + results[2]);
    } else {
        qDebug() << "AI output parsing error. Output was:" << output;
        if (ui->label_predicted_duration) ui->label_predicted_duration->setText("Durée Prédite : Erreur IA");
        if (ui->label_risk) ui->label_risk->setText("Risque de Retard : Erreur IA");
        if (ui->label_advice) ui->label_advice->setText("Conseil : Erreur IA");
    }
}
